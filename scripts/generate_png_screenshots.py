...
def render_dashboard_png(cfg):
    ...
    header_rect = patches.Rectangle(
        (0, 830), 1600, 70, facecolor="#1E293B", edgecolor="#334155", linewidth=1
    )
    ax.add_patch(header_rect)
    ax.text(
        30, 855, "EchoChain Lakehouse Analytics", color="#F8FAFC", fontsize=20, fontweight="bold"
    )
    ...
    for i, (label, val, color) in enumerate(cfg["kpis"]):
        ...
        card = patches.Rectangle(
            (x, 700), card_width, 105, facecolor="#1E293B", edgecolor="#334155", linewidth=1.5
        )
        ax.add_patch(card)
    ...
    v1 = patches.Rectangle(
        (50, 360), 730, 310, facecolor="#1E293B", edgecolor="#334155", linewidth=1.5
    )
    ax.add_patch(v1)
    ax.text(
        70,
        635,
        f"{cfg['title']} - Primary Metric Trend",
        color="#F8FAFC",
        fontsize=14,
        fontweight="bold",
    )
    ...
    v2 = patches.Rectangle(
        (810, 360), 740, 310, facecolor="#1E293B", edgecolor="#334155", linewidth=1.5
    )
    ax.add_patch(v2)
    ax.text(
        830,
        635,
        "Marketplace Platform Performance (eBay / FB / OLX / BackMarket)",
        color="#F8FAFC",
        fontsize=13,
        fontweight="bold",
    )
    ...
    platforms = [
        ("eBay", 540, "#10B981"),
        ("FB Marketplace", 460, "#06B6D4"),
        ("OLX", 380, "#8B5CF6"),
        ("BackMarket", 300, "#F59E0B"),
    ]
    ...
    v3 = patches.Rectangle(
        (50, 40), 1500, 290, facecolor="#1E293B", edgecolor="#334155", linewidth=1.5
    )
    ax.add_patch(v3)
    ax.text(
        70,
        295,
        "PySpark Medallion Delta Lakehouse Analytics Matrix & SKU Reconciliation",
        color="#F8FAFC",
        fontsize=14,
        fontweight="bold",
    )
