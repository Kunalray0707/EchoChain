...
def build_gold_metrics(spark, silver_dir="data/silver", gold_dir="data/gold"):
    ...
    df_bom_agg = (
        df_bom.groupby(["SKU", "Product", "Manufacturer", "Category"])
        .agg(
    ...
    df_gold_resale = df_bom_agg.merge(
        df_lst_agg, left_on="SKU", right_on="matched_sku", how="inner"
    )
    df_gold_resale["resale_index"] = (
        df_gold_resale["avg_resale_price_usd"] / df_gold_resale["total_mfg_cost_usd"]
    ).round(4)
    df_gold_resale["refurbishment_score"] = (
        (df_gold_resale["refurbished_count"] / df_gold_resale["total_listings_count"]) * 100
    ).round(2)
    df_gold_resale["landfill_diversion_pct"] = (
        (
            (df_gold_resale["total_listings_count"] - df_gold_resale["salvage_count"])
            / df_gold_resale["total_listings_count"]
        )
        * 100
    ).round(2)
    ...
    df_gold_resale["circularity_score"] = (
        (
            (clipped_resale_index * 0.5 + (df_gold_resale["landfill_diversion_pct"] / 100.0) * 0.5)
            * 100
        )
        .clip(lower=0.0, upper=100.0)
        .round(2)
    )
    df_gold_resale["co2_avoided_tons"] = (
        (df_gold_resale["total_listings_count"] * df_gold_resale["total_carbon_footprint_kg"] * 0.7)
        / 1000.0
    ).round(2)
    ...
    try:
        ...
        pass
    print(
        f"[OK] Gold Circularity & Resale Metrics Table Built: {len(df_gold_resale)} SKU summaries."
    )

    # 2. Gold Table: Component Failure
    df_war_agg = (
        df_war.groupby(["SKU", "failure_component"])
        .agg(
    ...
    df_gold_comp = df_bom.merge(
        df_war_agg, left_on=["SKU", "Component"], right_on=["SKU", "failure_component"], how="left"
    )
    ...
    df_gold_comp["repair_cost_ratio"] = (
        df_gold_comp["avg_repair_cost_usd"] / df_gold_comp["manufacturing_cost_usd"]
    ).round(2)
    df_gold_comp["repairability_index"] = (
        (10.0 - (df_gold_comp["claim_count"] / 100.0)).clip(lower=0.0).round(2)
    )
    ...
    df_gold_mkt = (
        df_lst.groupby(["marketplace", "normalized_condition", "location"])
        .agg(
    ...
    df_gold_mkt["total_sales_volume_usd"] = (
        df_gold_mkt["avg_price_usd"] * df_gold_mkt["total_sold_count"]
    ).round(2)
    ...
    df_gold_sust = (
        df_gold_resale.groupby(["Manufacturer", "Category"])
        .agg(
    ...
    df_gold_sust["carbon_financial_savings_usd"] = (
        df_gold_sust["total_co2_avoided_tons"] * 85.0
    ).round(2)
